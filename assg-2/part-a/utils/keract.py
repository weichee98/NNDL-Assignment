...
def display_activations(activations, cmap=None, save=False, directory='.', filename=None,
    ...
    :param directory: string - the name of the file (if save is True)
    ...
    for layer_name, acts in activations.items():
        ...
        if filename:
            fig.suptitle('{0} {1}'.format(filename, layer_name.split('/')[0]), fontsize=fig_size[1] * 2)
        else:
            fig.suptitle('{0} {1}'.format(index, layer_name.split('/')[0]), fontsize=fig_size[1] * 2)
        fig.tight_layout(rect=[0, 0, 1, 0.95])
        fig.subplots_adjust(right=0.8)
        cbar = fig.add_axes([0.85, 0.15, 0.03, 0.7])
        cbar = fig.colorbar(hmap, cax=cbar)
        cbar.ax.tick_params(labelsize=fig_size[1] * 1.5)
        if save:
            if not os.path.exists(directory):
                os.makedirs(directory)
            if filename:
                output_filename = os.path.join(directory, '{0}_{1}.png'.format(filename, layer_name.split('/')[0]))
            else:
