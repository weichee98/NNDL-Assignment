...
def compare():
    ...
    plt.title('MSE for Different Models Without Regularization', fontsize=15, pad=20)
    plt.xlim(500, 10000)
    ...
    df.applymap(lambda x: training_result(x, mode='loss')).to_csv('result/additional_compare.csv')
    logger.log('Saved result to \"result/additional_compare.csv\"')
