...
def rfe_n_features(n):
    ...
    cols = [c for c in df.columns if len(c.split(", ")) == n]
...
def compare():
    ...
    best_6 = val_mse[[c for c in df.columns if len(c.split(", ")) == 6]].idxmin()
    mse_6 = hist2['Q2'][best_6]['val_mse']
    best_5 = val_mse[[c for c in df.columns if len(c.split(", ")) == 5]].idxmin()
    ...
    if result == 'all_features':
        features_left = columns
    elif result == '6_features':
        features_left = best_6.split(", ")
    elif result == '5_features':
        features_left = best_5.split(", ")
    
    hyperparameters['input_shape'] = (len(features_left),)
    hyperparameters['features_left'] = features_left
